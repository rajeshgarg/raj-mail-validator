...
module ActiveModel
  module Validations
    class ValidAddressValidator < ActiveModel::EachValidator
      def validate_each(record, attribute, value)
        begin
           ...
           raise "not valid" unless value =~ reg
          domain = value.match(/\@(.+)/)[1]

          unless domain && domain.length > 1
            raise 
          end
          Resolv::DNS.open do |dns|
            @mx = dns.getresources(domain, Resolv::DNS::Resource::IN::MX) + dns.getresources(domain, Resolv::DNS::Resource::IN::A)
          end
          @mx.size > 0 ? true : record.errors.add(attribute, options[:message] || "Invalid Email Address")
        rescue
          record.errors.add(attribute, options[:message] || "Invalid Email Address")
        end
      end
    end
  end
